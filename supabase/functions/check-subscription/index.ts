...
serve(async (req) => {
  ...
  try {
    ...
    // Stale/expired token or signed-out session — treat as unauthenticated, not a server error
    if (userError || !userData?.user?.email) {
      logStep("Invalid or missing session — returning unsubscribed", { reason: userError?.message || "no user" });
      return new Response(JSON.stringify({ subscribed: false }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        status: 200,
      });
