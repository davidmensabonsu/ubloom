...
import { Sparkles, Heart, Feather, BookOpen, ChevronDown, Search, X, Calendar, Trash2, Infinity } from 'lucide-react';
...
export default function Alignment() {
  ...
  const filteredEntries = useMemo(() => {
    return profile.journalEntries.filter((entry) => {
      ...
      const matchesSearch =
      searchQuery.trim() === '' || entry.content.toLowerCase().includes(searchQuery.toLowerCase());
                  ...
                  <motion.div
                    ...
                    <div className="mt-2 pt-2 border-t border-primary/10">
                              <span className="text-xs text-muted-foreground flex items-center gap-1">
                                Mood:{' '}
                                {feelingIcons[entry.mood] &&
                        <img src={feelingIcons[entry.mood]} alt="" className="w-4 h-4 object-contain inline" style={{ filter: 'none' }} />
                        }{' '}
